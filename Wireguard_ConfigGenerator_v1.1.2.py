...
if __name__ == '__main__':
    ...
    rawentries = readEntriesList(pathToHosts)

    entries = []
    for x in rawentries:
        try:
            if x[0] not in [x[0] for x in entries]:
                entries.append(x)
        except:
            pass
    ...
    for x in os.listdir(keysRepoName):
        with open(f"{keysRepoName}/{x}/privatekey", "r") as privateKeyData:
            allData[x]["PrivateKey"] = privateKeyData.read().replace("\n","")
        with open(f"{keysRepoName}/{x}/publickey", "r") as publicKeyData:
            allData[x]["PublicKey"] = publicKeyData.read().replace("\n","")
